...
if __name__ == "__main__":
    ...
    import json
    split = read_split(r"D:\testing_AI_environment\Autopet\splits_final.json", 0)
    prediction_folder = {"train": r"D:\testing_AI_environment\results\train\predicted_images",
                         "val": r"D:\testing_AI_environment\results\val\predicted_images"}
    ground_truth_folder = r"D:\testing_AI_environment\Autopet\labelsTr"
    organ_segmentation_folder = r"D:\testing_AI_environment\results\SegResNet"
    metrics_folder = {"train": r"D:\testing_AI_environment\results\train\metrics_per_organ",
                        "val": r"D:\testing_AI_environment\results\val\metrics_per_organ"}
    ...
    processed_files = json.load(open(os.path.join(metrics_folder["val"], "processed_list_Surface.json"), "r")) + json.load(open(os.path.join(metrics_folder["val"], "processed_list_Air.json"), "r")) + json.load(open(os.path.join(metrics_folder["val"], "processed_list.json"), "r"))
    for train_or_val in ["val"]:
        for file in tqdm([file for file in split[train_or_val] if file not in processed_files], desc=f"Processing {train_or_val} data"):
            prediction_path = os.path.join(prediction_folder[train_or_val], file + "_0000_pred.nii.gz")
            ...
            json.dump(metrics, open(os.path.join(metrics_folder[train_or_val], file + "_metrics.json"), "w"), default=default)
            processed_files.append(file)
            json.dump(processed_files, open(os.path.join(metrics_folder["val"], "processed_list.json"), "w"))
