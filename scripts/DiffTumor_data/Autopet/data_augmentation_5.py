...
if __name__ == "__main__":
    ...
    files = divide_list(files, n=8)[0]
    for i in reversed(range(len(files))):
        file_entry = files[i]
        if file_entry["ct"].split('/')[-1].split(".nii")[0] + f"{args.samples_per_file-1:02}_synthesized.nii.gz" in list(os.listdir(dest)):
