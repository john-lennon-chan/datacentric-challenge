...
import sys
sys.path.append(r'/scripts/DiffTumor/STEP3_SegmentationModel')
...
class LightningGenerator(L.LightningModule):
    def __init__(self):
        ...
        def sliding_window(synthesize_stage_tumour):
            def sliding_window_version(healthy_data, healthy_target, *args, **kwargs):
                def new_synthesize_stage_tumour(concatenated_image, *args, **kwargs):
                    synt_data, synt_target = synthesize_stage_tumour(concatenated_image[:, 0:2, :, :, :],
                                                                     concatenated_image[:, 2:3, :, :, :], *args,
                                                                     **kwargs)
        ...
        def synthesize_tumour(self, batch_data, vqgan, early_sampler, noearly_sampler, args):
            ...
            if synthetic_tumor_type == 'early':
                synt_data, synt_target = synthesize_early_tumor(healthy_data, healthy_target, healthy_original_label,
                                                                args.organ_type, vqgan,
                                                                early_sampler, args)
            elif synthetic_tumor_type == 'medium':
                synt_data_list, synt_target_list = [], []
                for i in range(healthy_data.shape[0]):
                    synt_data, synt_target = synthesize_medium_tumor(healthy_data[i:i + 1], healthy_target[i:i + 1],
                ...
                synt_data_list, synt_target_list = [], []
                for i in range(healthy_data.shape[0]):
                    synt_data, synt_target = synthesize_large_tumor(healthy_data[i:i + 1], healthy_target[i:i + 1],
                                                                    healthy_original_label[i:i + 1], args.organ_type,
                                                                    vqgan,
                                                                    noearly_sampler, ddim_ts=args.ddim_ts, args=args)
                    synt_data_list.append(synt_data[0])
        ...
        def synthesize_tumour_transform(data, synthesize_tumour, *args, **kwargs):
            if isinstance(data, dict):
                data["image"], data["label"] = synthesize_tumour(data, *args, **kwargs)
                return data
            elif isinstance(data, list):
                # return [self.__call__(d) for d in data]
                data_stack = {k: torch.stack(tuple([d[k] for d in data])) for k in
                              ["image", "label", "original_label"]}
                data_stack["image"], data_stack["label"] = synthesize_tumour(data_stack, *args,
                                                                                  **kwargs)
    ...
    def forward(self, batch):
        ...
        data = batch
        data = self.synthesize_tumour_transform(data, self.synthesize_tumour, vqgan, early_sampler, noearly_sampler, args)
...
dest = "/data/lychan/24_Summer_Research/datacentric-challenge/scripts/DiffTumor_data/Autopet/imagesTr_Step_3_synthesized_15/"
worker = 10
...
class Args:
    def __init__(self):
        ...
        self.ddim_ts = 50
        self.output_dir = "/scripts/DiffTumor_data/Autopet/imagesTr_Step_3_synthesized_15/"
...
if __name__ == "__main__":
    ...
    files = creating_data_dicts(root,
                                "/data/lychan/24_Summer_Research/datacentric-challenge/scripts/DiffTumor_data/Autopet/Step_3_synthesize_datalist_all.txt")
    ...
    ds = AugmentedDataset(root, dest, files, synthesize_transform=synthesize_transform, samples_per_file=args.samples_per_file, seed=seed, resume=True, args=args)

    dataloader = DataLoader(ds, batch_size=1, shuffle=False, num_workers=worker)

    model = LightningGenerator()
