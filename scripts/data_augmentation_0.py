    ...
    def sliding_window_version(healthy_data, healthy_target, *args, **kwargs):
        def new_synthesize_stage_tumour(concatenated_image, *args, **kwargs):
            synt_data, synt_target = synthesize_stage_tumour(concatenated_image[:, 0:2, :, :, :],
                                                             concatenated_image[:, 2:3, :, :, :], *args, **kwargs)
...
def synthesize_tumour(batch_data, vqgan, early_sampler, noearly_sampler, args):
    ...
    if synthetic_tumor_type == 'early':
        ...
        synt_data, synt_target = synthesize_medium_tumor(healthy_data, healthy_target, healthy_original_label,
                                                         args.organ_type, vqgan,
                                                         noearly_sampler, ddim_ts=args.ddim_ts, args=args)
    elif synthetic_tumor_type == 'large':
        synt_data, synt_target = synthesize_large_tumor(healthy_data, healthy_target, healthy_original_label,
                                                        args.organ_type, vqgan,
                                                        noearly_sampler, ddim_ts=args.ddim_ts, args=args)
...
class SynthesizeTransform(transforms.Transform):
    ...
    def __call__(self, data):
        ...
        if isinstance(data, dict):
            ...
            data_concat = {k: torch.cat(tuple([d[k] for d in data]), 0) for k in ["image", "label", "original_label"]}
            data_concat["image"], data_concat["label"] = self.synthesize_tumour(data_concat, *self.args, **self.kwargs)
            for i, d in enumerate(data):
                d["image"] = data_concat["image"][i]
                d["label"] = data_concat["label"][i]
                d["original_label"] = data_concat["original_label"][i]
            return data
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return data#transformed_synthesized_data
...
if __name__ == "__main__":
    ...
    for data in tqdm(dataloader, total=len(dataloader)):
        assert(data==0)
        vqgan, early_sampler, noearly_sampler = synthesize_tumour_preparation(args)
        data = SynthesizeTransform(synthesize_tumour, vqgan, early_sampler, noearly_sampler, args)(data)
        transformed_synthesized_data = post_synthesize_transform(data)
