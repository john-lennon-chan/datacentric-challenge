...
class AugmentedDataset(Dataset):
    ...
    def synthesize_tumour(self, batch_data, j, vqgan, early_sampler, noearly_sampler, args):
        ...
        if synthetic_tumor_type == 'early':
            synt_data, synt_target = synthesize_early_tumor(healthy_data, healthy_target, args.organ_type, vqgan,
                                                            early_sampler, args)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.samples_per_file):
            ...
            data[i] = self.synthesize_tumour_transform(transformed_data[i], i, vqgan, early_sampler, noearly_sampler, args)
            #print("checkpoint 2")
...
if __name__ == "__main__":
    ...
    def divide_list(lst):
        ...
        # Use list slicing to create the 4 parts
        part1 = lst[:size]
        part2 = lst[size:2 * size]
        part3 = lst[2 * size:3 * size]
        part4 = lst[3 * size:]

        return part1, part2, part3, part4

    files_chuncks = divide_list(files)
    ...
    #for i, files in enumerate(files_chuncks):
    #    args.rank = i
    ...
    def process_files_on_gpu(gpu_id, files, args):
        ...
        # Create the AugmentedDataset and DataLoader
        synthesize_transform, _ = _get_synthesize_transform(args)
        post_synthesize_transform, _ = _get_post_synthesize_transform(args)
        ds = AugmentedDataset(root, dest, files, synthesize_transform=synthesize_transform,
                              post_synthesize_transform=post_synthesize_transform,
                              samples_per_file=args.samples_per_file, seed=seed, resume=True, args=args)
        dataloader = DataLoader(ds, batch_size=1, shuffle=False, num_workers=worker)
    ...
    def main(files_chunks=files_chuncks, args=args):

        # Create a process for each GPU
        processes = []
        for i in range(4):  # Adjust this to the number of GPUs you want to use
            p = multiprocessing.Process(target=process_files_on_gpu, args=(i, files_chunks[i], args))
            p.start()
            processes.append(p)
    ...
    main(files_chunks=files_chuncks, args=args)
